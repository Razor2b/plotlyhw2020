// function for plotting bar, gauge, bubble
...
function getPlot(id) {
    ...
    d3.json("data/samples.json").then((data)=> {
        ...
        var layout = {
            title: "Top 10 OTU for Test Subject",
            yaxis:{
                tickmode:"linear",
            },
            margin: {
        ...
        var layout_b = {
            title: "Bectria for Test Subject",
            xaxis:{title: "OTU ID"},
            height: 600,
            width: 1000
        };
        ...
        // create guage chart for wash freq
  
        var data_g = [
          {
          domain: { x: [0, 1], y: [0, 1] },
          value: parseFloat(wfreq[sample_index]),
          title: { text: `Weekly Washing Frequency of Test Subject` },
          type: "indicator",
...
function getInfo(id) {
    ...
    d3.json("data/samples.json").then((data)=> {
        ...
        // select table to put data
        var demographicInfo = d3.select("#sample-metadata");
        
        // empty the table each time before getting new info
        demographicInfo.html("");

        // grab the necessary demographic data data for the id and append the info to the table
